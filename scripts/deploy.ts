...
async function dockerLogin(): Promise<void> {
  ...
  if (!username || !token) {
    console.log(
      'DOCKERHUB_USERNAME / DOCKERHUB_TOKEN not set — skipping docker login (assuming already logged in)',
    )
    return
  }

  console.log(`\nLogging in to Docker Hub as ${username}…`)
  const proc = Bun.spawn(['docker', 'login', '-u', username, '--password-stdin'], {
    stdin: 'pipe',
    stdout: 'inherit',
    stderr: 'inherit',
  })
